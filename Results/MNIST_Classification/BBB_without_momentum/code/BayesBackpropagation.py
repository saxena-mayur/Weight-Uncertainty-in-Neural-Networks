...
def log_gaussian(x, mu, sigma):
    return -0.5 * np.log(2 * np.pi) - torch.log(torch.abs(sigma)) - (x - mu) ** 2 / (2.0 * sigma ** 2)
...
class BayesianLinear(nn.Module):
    ...
    def forward(self, input, infer=False):
        ...
        if self.hasScalarMixturePrior:  # for Scalar mixture vs Gaussian analysis
            ...
            self.lpw = log_gaussian(weight, 0, self.SIGMA_2).sum() + log_gaussian(bias, 0, self.SIGMA_2).sum()

        self.lqw = log_gaussian(weight, self.weight_mu, weight_sigma).sum() + log_gaussian(bias, self.bias_mu, bias_sigma).sum()
